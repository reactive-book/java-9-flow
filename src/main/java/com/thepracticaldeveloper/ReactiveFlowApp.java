...
public class ReactiveFlowApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    log.info("\n\n### CASE 4: Same as CASE 1, but Pete has a naughty dog " +
      "keeping some magazines for himself...");
    final FlexibleProcessor<Integer, Integer> dog = new FlexibleProcessor<>(
      Function.identity(), // at least the dog does not modify the numbers...
      (magazineNumber) -> magazineNumber % 5 != 0
    );
    app.magazineDeliveryExample(100L, 100L, 8, dog);

  }

  private void magazineDeliveryExample(final long sleepTimeJack,
                                       final long sleepTimePete,
                                       final int maxStorageInPO) throws Exception {
    magazineDeliveryExample(sleepTimeJack, sleepTimePete, maxStorageInPO, null);
  }

  void magazineDeliveryExample(final long sleepTimeJack,
                               final long sleepTimePete,
                               final int maxStorageInPO,
                               final Flow.Processor<Integer, Integer> petesDog)
    throws Exception {
    ...
    if (petesDog != null) {
      publisher.subscribe(petesDog);
      petesDog.subscribe(pete);
    } else {
